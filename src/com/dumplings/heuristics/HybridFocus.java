...
public class HybridFocus extends AbstractHeuristic implements PlayerHeuristic {
	private StateMachine stateMachine = null;
	private List<Role> roles;
	private Random generator;
	private MachineState currentState;
	
	public HybridFocus(StateMachine stateMachine) {
	...
	public Integer getScore(MachineState state, Role role) throws MoveDefinitionException, TransitionDefinitionException, GoalDefinitionException {
		...
		if (stateMachine.isTerminal(currentState)) { // oh, hey, this is actually a goal state. yippee!
			...
			double movesRatio = -(moves.size() - avgOpponentMoves) / (moves.size() + avgOpponentMoves); // -1 => self more mobile, 0 => balanced moves, 1 => opponent(s) more mobile
